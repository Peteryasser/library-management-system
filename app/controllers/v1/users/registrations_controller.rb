...
class V1::Users::RegistrationsController < Devise::RegistrationsController
  ...
  def create
    ...
    resource.can_borrow = true
    resource.save
    if resource.persisted?
      otp = resource.generate_otp
      UserMailer.otp_email(resource, resource.email, otp).deliver_now
  ...
  def sign_up_params
    params.require(:user).permit(:user_name, :email, :password, :password_confirmation, :name, :role, :status)
  end
